...
if __name__ == "__main__":
    ...
    parser.add_argument('-c', '--camera', type=int, default=0, help='摄像头ID (默认: 4)')
    parser.add_argument('-t', '--time', type=int, default=60, help='测试持续时间，单位秒 (默认: 60，设为0表示无限时长)')
    parser.add_argument('-l', '--list', action='store_true', help='列出可用摄像头')
    parser.add_argument('-m', '--max', type=int, default=10, help='扫描摄像头的最大数量 (默认: 10)')
    
    args = parser.parse_args()
    
    if args.list:
        list_available_cameras(args.max)
    else:
        test_camera(camera_id=args.camera, duration=args.time) 